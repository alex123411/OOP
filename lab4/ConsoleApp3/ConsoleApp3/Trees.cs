...
namespace ConsoleApp2_Lab2
{
    ...
    class Trees : Plants, BigTree, SmallTree
    {
        ...
        {
            Disposing(false);
            GC.SuppressFinalize(this);    //supressFinilize
        }
        private void Disposing(bool f)
        {
            if (!disposed)
            {
                if (f)
                {
                    Console.WriteLine("Tree's controled resourses would be cleaned");
                }
                Console.WriteLine("Tree's discontroled resourses would be cleaned");
                disposed = true;
            }
        }
        ~Trees()
